...
func (c *Clickhouse) GetReport(
	...
	template := "SELECT eventTime, if(platform = 'web', 'mav', platform) as platform, count(*) as value " +
		"FROM visitors_1_day_mv " +
		"WHERE eventTime >= ? AND eventTime <= ? %s " +
		"GROUP BY platform, eventTime " +
